...
const userAgent = `elasticsearch-js/${clientVersion} (${os.platform()} ${os.release()}-${os.arch()}; Node.js ${
  process.version
})`;
...
class Transport {
  constructor(opts) {
    if (typeof opts.compression === "string" && opts.compression !== "gzip") {
      throw new ConfigurationError(
        `Invalid compression: '${opts.compression}'`
      );
    }
    ...
    this[kMaxCompressedResponseSize] =
      opts.maxCompressedResponseSize || MAX_BUFFER_LENGTH;
  ...
  request(params, options, callback) {
    ...
    const maxCompressedResponseSize =
      options.maxCompressedResponseSize || this[kMaxCompressedResponseSize];
    ...
    const onConnectionError = (err) => {
      if (err.name !== "RequestAbortedError") {
        ...
        if (meta.attempts < maxRetries) {
          meta.attempts++;
          debug(
            `Retrying request, there are still ${
              maxRetries - meta.attempts
            } attempts`,
            params
          );
          makeRequest();
    ...
    const onResponse = (err, response) => {
      ...
      const isVectorTile =
        (result.headers["content-type"] || "").indexOf(
    ...
    const onBody = (err, payload) => {
      ...
      if (
        ...
        result.body =
          isHead === true && result.statusCode < 400 ? true : payload;
      }
      ...
      const ignoreStatusCode =
        (Array.isArray(options.ignore) &&
          options.ignore.indexOf(result.statusCode) > -1) ||
  ...
  sniff(opts, callback = noop) {
    ...
    this.request(request, { id: opts.requestId }, (err, result) => {
      ...
      const protocol =
        result.meta.connection.url.protocol ||
        /* istanbul ignore next */ "http:";
      const hosts = this.connectionPool.nodesToHost(
        result.body.nodes,
        protocol
      );
    ...
    this.request(
      ...
      (err, result) => {
        ...
        if (err) {
          ...
          if (
            result.body.version == null ||
            typeof result.body.version.number !== "string"
          ) {
